...
function jsonFetcher(byUrl: Record<string, unknown>): {
	fetcher: HttpFetcher;
	urls: string[];
} {
	const urls: string[] = [];
...
const ARCHIVE_PAGE: unknown = [
	{
		...
		canonical_url:
			'https://jonathanmclernon.substack.com/p/older-free-post',
		audience: 'everyone',
		type: 'newsletter',
		section_name: 'Essays',
...
describe('SubstackFeedSource', () => {
	it('declares its source type', () => {
		expect(buildSource(fixture('substack-multi.xml')).type).toBe(
			'substack',
		);
	});
	...
	it('resolve() builds a ResolvedFeed with publicationTitle and sampleTitles', async () => {
		...
		const feed = await source.resolve(
			'https://jonathanmclernon.substack.com/feed',
		);
	...
	it('skips archive entries with neither an id nor a canonical url', async () => {
		const malformed: unknown = [
			{ title: 'No identity at all' },
			{
				...
				slug: 'kept',
				title: 'Kept',
				canonical_url: 'https://x.test/p/kept',
			},
		];
	...
	it('fetchBody() on an archive item fetches the post and sets contentHtml', async () => {
		const postUrl =
			'https://jonathanmclernon.substack.com/api/v1/posts/older-free-post';
		const { fetcher, urls } = jsonFetcher({
			[ARCHIVE_URL]: ARCHIVE_PAGE,
			[postUrl]: {
				body_html:
					'<div class="body"><p>The full older free body.</p></div>',
			},
		});
		const source = new SubstackFeedSource({
			fetcher,
	...
	it('fetchBody() returns the archive item unchanged when the post fetch fails', async () => {
		...
		const source = new SubstackFeedSource({
			fetcher: failing,
			resolver: new FeedResolver(failing),
		});
		const items = await source.listItems(SUBSTACK_FEED, {
			offset: 20,
			limit: 12,
