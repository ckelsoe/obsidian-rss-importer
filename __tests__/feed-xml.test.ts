...
describe('parseFeed', () => {
	describe('RSS Substack item', () => {
		...
		it('reads guid, link, and title', () => {
			...
			expect(item.title).toBe(
				'If I am not saved by the system, then how am I saved?',
			);
		});
	...
	describe('Atom entry', () => {
		...
		it('reads <content> as contentHtml and <summary> as description', () => {
			expect(item?.contentHtml).toBe(
				'<p>The full Atom body content lives here.</p>',
			);
	...
	describe('malformed and rootless input', () => {
		it('throws FeedParseError on malformed XML', () => {
			expect(() => parseFeed('<rss><channel><title>oops</rss>')).toThrow(
				FeedParseError,
			);
		});
		...
		it('throws FeedParseError when there is no channel or feed root', () => {
			const xml =
				'<?xml version="1.0"?><html><body><p>not a feed</p></body></html>';
