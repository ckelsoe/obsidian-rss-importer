...
describe('detectPaywall', () => {
	...
	describe('structural truncation detection', () => {
		it('flags a div.paywall wrapper as truncated and reports the signal', () => {
			const html =
				'<article><p>Free preview line.</p><div class="paywall">Subscribe to keep reading</div></article>';
			const result = detectPaywall({
				audienceField: 'everyone',
				bodyHtml: html,
			});
		...
		it('flags a subscribe-widget wrapper as truncated', () => {
			const html =
				'<article><p>Preview.</p><div class="subscribe-widget">Subscribe</div></article>';
			const result = detectPaywall({
				audienceField: null,
				bodyHtml: html,
			});
		...
		it('does NOT flag an unknown-audience post with prose subscribe and no wrapper', () => {
			const html =
				'<article><p>Remember to subscribe and share. Members get extras.</p></article>';
			const result = detectPaywall({
				audienceField: null,
				bodyHtml: html,
			});
	...
	describe('paid plus short body', () => {
		it('flags a paid item with a short body via the truncation marker', () => {
			const html =
				'<article><p>Teaser.</p><div class="available-content">cut off here</div></article>';
			const result = detectPaywall({
				audienceField: 'only_paid',
				bodyHtml: html,
			});
		...
		it('flags a paid item with a short plain body (no wrapper) via paid-short-body', () => {
			const html =
				'<article><p>This paid teaser is just a couple of lines.</p></article>';
			const result = detectPaywall({
				audienceField: 'only_paid',
				bodyHtml: html,
			});
		...
		it('does NOT flag a paid item with a long complete body and no wrapper', () => {
			const longBody =
				'This is a full paid post with plenty of body text. '.repeat(
					40,
				);
