 ...
 */
export function defaultDestinationFolder(
	parentFolder: string,
	publicationTitle: string,
): string {
	const parent = trimChars(parentFolder.trim(), '/');
	const title = trimChars(
		publicationTitle.trim().replace(/[\\/]+/g, ' '),
		'/',
	).trim();
	if (parent.length === 0) {
		return title;
...
export class AddFeedModal extends Modal {
	...
	onOpen(): void {
		...
		const inputRow = createStackedRow(contentEl, {
			...
			description:
				'A feed URL, a Substack subdomain or post URL, or an @handle.',
		});
		...
		this.previewEl = previewRow.content.createDiv({
			cls: 'rss-importer-preview',
		});
		...
		const folderInput = folderRow.content.createEl('input', {
			...
			attr: {
				type: 'text',
				placeholder: this.deps.settings.defaultParentFolder,
			},
		});
	...
	private async resolveInput(): Promise<void> {
		...
		try {
			...
			new Notice(
				'Could not resolve that feed. Check the URL or handle and try again.',
			);
			console.error(err);
		}
	}
	...
	private renderPreviewCard(resolved: ResolvedFeed): void {
		...
		header.createSpan({
			...
			text:
				resolved.publicationTitle.length > 0
					? resolved.publicationTitle
		...
		meta.createSpan({
			cls: 'rss-importer-preview-host',
			text: resolved.canonicalHost,
		});
		...
		if (resolved.author !== null && resolved.author.length > 0) {
			meta.createSpan({
				cls: 'rss-importer-preview-author',
		...
		if (resolved.sampleTitles.length > 0) {
			const list = el.createEl('ul', {
				cls: 'rss-importer-preview-samples',
			});
